...
(def test-cases @{
  ...
  "explicit fresnel" `(box 75 :r 10 | color (let [view-dir (normalize (- camera P))] (1.0 - dot normal view-dir | pow 5 + c)))`
  ...
  "normal colors" `(sphere 50 | shade (remap+ normal))`
  "perlin noise 2d" `(box 100 :r 10 | color [0 (perlin+ (* 0.1 p.xz)) 0])`
  "perlin noise 3d" `(box 100 :r 10 | color [0 (perlin+ (* 0.1 p)) 0])`
  "perlin noise 4d" `(box 100 :r 10 | color [0 (perlin+ (* 0.1 p.xyzx)) 0])`
...
(each [name program] (sort (pairs test-cases) (fn [[name1 _] [name2 _]] (< name1 name2)))
  ...
  (if (os/stat final-file-name)
    (do
      (eprintf "%s already exists" final-file-name)
      (os/rm temporary-file-name))
    (do
      (eprintf "%s does not exist already" final-file-name)
