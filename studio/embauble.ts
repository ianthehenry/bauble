...
class Renderer {
  ...
  constructor(canvas: HTMLCanvasElement, private state: {
    ...
    const left = -0.5 * resolution[0];
    const right = 0.5 * resolution[0];
    const top = 0.5 * resolution[1];
    const bottom = -0.5 * resolution[1];
...
export default function Bauble(canvas: HTMLCanvasElement, opts: {
  ...
  const {source, isAnimated, resolution} = opts;
